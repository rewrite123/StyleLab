...
export const aggregateComps = {
  changeStyleLabsVar: styleLabFn,
  Button: Button
};

window.addEventListener('load', ()=>{
  const targetRender = document.getElementsByClassName('sl');
  // debugger;
  if(targetRender.length > 0){
    const root = createRoot(targetRender[0]);
    root.render(React.createElement(Button, {size: 'lg', shadow: 4, variant: 'primary'}, 'Other test'));
  }
});